...
##############################################################################################
    ...
    def check_win_v3(self, id_number):
        while True:
            try:
                order_data = self.api.order_async[id_number].get("position-changed")
                if order_data and order_data.get("msg", {}).get("status") == "closed":
                    msg = order_data["msg"]
                    reason = msg.get("close_reason", "unknown")
                    pnl = msg.get("pnl_realized", 0.0)
                    
                    if reason == "win":
                        return "win", pnl
                    elif reason in ("loose", "loss", "lose"):
                        return "loose", pnl
                    elif reason in ("equal", "tie", "empty"):
                        return "equal", pnl
                    else:
                        if pnl > 0: return "win", pnl
                        elif pnl < 0: return "loose", pnl
                        else: return "equal", pnl
            except:
                pass
            time.sleep(0.5)
